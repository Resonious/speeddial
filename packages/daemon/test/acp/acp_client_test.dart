...
void main() {
  ...
  test(
    'newSession and loadSession forward MCP server configurations',
    () async {
      File(p.join(tempDir.path, 'agent.capture_mcp')).writeAsStringSync('');
      final AcpClient client = spawnClient(cwd: tempDir.path);
      addTearDown(client.dispose);
      ...
      final List<Map<String, Object?>> servers = <Map<String, Object?>>[
        <String, Object?>{
          ...
          'args': <String>['_internal-mcp'],
          'env': <Object?>[
            <String, Object?>{'name': 'TOKEN', 'value': 'new'},
          ],
        },
      ];
      ...
      final List<Map<String, Object?>> loadServers = <Map<String, Object?>>[
        <String, Object?>{
          ...servers.single,
          'env': <Object?>[
            <String, Object?>{'name': 'TOKEN', 'value': 'load'},
          ],
        },
      ];
  ...
  test(
    'cancel resolves the pending prompt with the cancelled stop reason',
    () async {
  ...
  test(
    'unknown session update variants are dropped without crashing',
    () async {
      final client = spawnClient(targetPath: targetPath);
      addTearDown(client.dispose);
  ...
  test(
    'loadSession surfaces an agent error when the session is unknown',
    () async {
      ...
      await expectLater(
        ...
        throwsA(
          isA<AcpJsonRpcException>().having((e) => e.code, 'code', -32000),
        ),
      );
    },
  );

  test('loadSession capability is absent when the agent declines it', () async {
  ...
  test('newSession and loadSession report configOptions; setConfigOption '
    ...
    final model = created.configOptions.firstWhere(
    ...
    expect(model.options.map((option) => option.value), <String>[
      'fake-fast',
      'fake-smart',
    ]);
    final thinking = created.configOptions.firstWhere(
    ...
    expect(thinking.options.map((option) => option.value), <String>[
      'off',
      'auto',
      'low',
      'high',
    ...
    expect(loaded.map((option) => option.id), <String>['model', 'thinking']);
    expect(
    ...
    await expectLater(
      ...
      throwsA(isA<AcpJsonRpcException>().having((e) => e.code, 'code', -32602)),
    );
  });
  ...
  test('configOptions omit the thinking option and reject its config id '
    ...
    expect(created.configOptions.map((option) => option.id), <String>['model']);
    await expectLater(
  ...
  test('configOptions omit the model option and reject its config id '
    ...
    expect(created.configOptions.map((option) => option.id), <String>[
      'thinking',
    ]);
    await expectLater(
