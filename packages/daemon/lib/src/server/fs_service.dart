...
///
/// Implements PROTOCOL.md `fs.list` / `fs.read`: paths are relative to the
/// project root, absolute paths are rejected, and anything resolving outside
/// the root is an invalid-params error (`-32602`) — never a path traversal.
library;
...
class FsService {
  ...
  String resolveInRoot(String rootPath, String requested) {
    ...
    final root = p.canonicalize(rootPath);
    final candidate = p.canonicalize(p.join(root, requested));
